...
import re
import nltk  # needs to install it: python -m nltk.downloader all
...
def clean_word(word):
    if word is not None and word != '':
        ...
        filtering_words = pd.read_csv(gbls.cleaning_words_path , sep=",", header=None)[0:].values.tolist()[0]
...
class CVEMatcher:
    ...
    def __init__(self, df_assets, df_cves):
        ...
        stanza.download('en', gbls.stanza_path) # added to use stanza package
        self.nlp = stanza.Pipeline('en', processors='tokenize,pos', dir=gbls.stanza_path)
    ...
    def find_matched_cves_to_assets(self):
        ...
        df_assets_cpe.to_csv(gbls.nvddir + "df_assets_cves.csv")

        if not df_assets_cpe.empty:
            ###uncoment this, if you need one row per cve
            # df_assets_cpe = df_assets_cpe.join(df_assets_cpe['cves'].apply(pd.Series).stack().reset_index(1, name='cve')).fillna('').drop('level_1', 1)
            ...
            # df.groupby('product').agg(lambda x: x.tolist()))
    ...
    def save_matched_cves(self):
        ...
        if not self.matched_cves.empty:
            self.matched_cves.to_csv(gbls.matched_nvd_cves)
        ...
        if not self.matched_cves.empty:
            try:
                ...
                self.matched_assets['vendor'] = self.matched_assets['vendor'].apply(
                    lambda x: re.sub(pattern=r"\x00", string=x, repl=' ') if x!=None else '')
                self.matched_assets.to_csv(gbls.matched_assets)
            except:
                print(f"Error in encoding data and writing data to csv: {yesterday.strftime('%Y%m%d')}")

        else:
            print(f"Matched_CVEs was empty for data: {yesterday.strftime('%Y%m%d')}")
...
def is_product_in_cve_summary(product, summary_nouns):
    if not summary_nouns or not product:
        ...
        #a = False if not (set(product_nouns).intersection(set(summary_nouns))) else True
